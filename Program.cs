...
using (var scope = app.Services.CreateScope())
{
    ...
    {
        Console.WriteLine("[DATABASE] WARNING: Standard migration failed, attempting hardened survival mode: " + ex.Message);
    }

    // HARDENED TABLE CHECK (UserGear) - ALWAYS RUNS
    try 
    {
        var createTableSql = @"
            CREATE TABLE IF NOT EXISTS ""UserGear"" (
                ...
                CONSTRAINT ""PK_UserGear"" PRIMARY KEY (""Id""),
                CONSTRAINT ""FK_UserGear_Users_UserId"" FOREIGN KEY (""UserId"") REFERENCES ""Users"" (""Id"") ON DELETE CASCADE
            );
            CREATE INDEX IF NOT EXISTS ""IX_UserGear_UserId"" ON ""UserGear"" (""UserId"");";
        
        db.Database.ExecuteSqlRaw(createTableSql);
        Console.WriteLine("[DATABASE] OK: Hardened check for 'UserGear' completed.");
    ...
    {
        Console.WriteLine("[DATABASE] CRITICAL ERROR in hardened sync: " + ex.Message);
    }
}
